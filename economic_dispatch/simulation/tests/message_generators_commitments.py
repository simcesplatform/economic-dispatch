...
from economic_dispatch.simulation.dataclasses import LFMResult

from datetime import datetime

# Maybe use readymade procemplus blocks etc instead
...
class ManagerMessageGenerator(MessageGenerator):
    ...
    def __init__(self, simulation_id: str, process_id: str, start_time: datetime):
        """
...
class LFMMarketResultMessageGenerator(MessageGenerator):
    """
    Message generator for LFMMarketResult messages.
    Input as list of length epochs.
        If list element None => empty results
        If list element TimeSeriesBlock => LFMMarketResult
    """
    ...
    def get_lfm_result_message(self, epoch_number: int, triggering_message_ids: List[str]) -> \
            Union[LFMMarketResultMessage, None]:
...
time_str = "2020-01-01T10:00:00.000Z"
TIME_INDEX = list()
for val in range(0, HORIZON+EPOCHS-1):
    TIME_INDEX.append(to_iso_format_datetime_string(to_utc_datetime_object(time_str) + timedelta(hours=val)))

BATT_DISPATCH_VALUES = [10.0]*(HORIZON+EPOCHS-1)
MARKET_DISPATCH_VALUES = [5.0]*(HORIZON+EPOCHS-1)

GEN_FORECAST_VALUES = [5.0]*(HORIZON+EPOCHS-1)
LOAD_FORECAST_VALUES = [-20.0]*(HORIZON+EPOCHS-1)
PRICE_FORECAST_VALUES = [1.0]*(HORIZON+EPOCHS-1)
STORAGE_STATE_VALUES = [70.0]*(HORIZON+EPOCHS-1)

# _REQUEST = Request(congestion_id="cid1", customer_ids=["id1"], activation_time="2020-01-01T02:00:00.000Z", duration=60,
#                    direction="downregulation", real_power_min=1.0, real_power_request=5.0, bid_resolution=1.0)
# _REQUEST2 = Request(congestion_id="cid2", customer_ids=["id1"], activation_time="2020-01-01T03:00:00.000Z", duration=60,
#                     direction="downregulation", real_power_min=1.0, real_power_request=6.0, bid_resolution=1.0)
# REQUESTS = [None, _REQUEST, _REQUEST2, None, None, None, None, None]

# _LFM_RESULT = LFMResult(congestion_id="cid1", customer_ids=["id1"], activation_time="2020-01-01T02:00:00.000Z",
#                         duration=60,
#                         direction="downregulation", real_power=2.0, price=1.0, offer_id="ED-test-flex-cid1-1")
# _LFM_RESULT2 = LFMResult(congestion_id="cid1", customer_ids=["id1"], activation_time="2020-01-01T02:00:00.000Z",
#                          duration=60,
#                          direction="downregulation", real_power=3.0, price=1.0, offer_id="ED-test-flex-cid1-2")
# _LFM_RESULT3 = LFMResult(congestion_id="cid2", customer_ids=["id1"], activation_time="2020-01-01T03:00:00.000Z",
#                          duration=60,
#                          direction="downregulation", real_power=1.0, price=1.0, offer_id="ED-test-flex-cid2-1")
# LFM_RESULTS = [None, None, [_LFM_RESULT], [_LFM_RESULT, _LFM_RESULT3], [_LFM_RESULT3], None, None, None]

LFM_RESULTS = [None]*HORIZON
