...
from economic_dispatch.simulation.component import create_component, SCENARIO_JSON, RESOURCE_TYPE, \
    RESOURCE_STATE_TOPIC, PRICE_FORECAST_STATE_TOPIC, RESOURCE_FORECAST_TOPIC, DISPATCH_SOLVER, \
    COMMITMENT_TIME, MARKET_ID, LFM_RESULT_TOPIC, CUSTOMER_INFO_TOPIC, STATUS_TOPIC, SKIP_OPEN_OFFERS
from domain_messages.dispatch import ResourceForecastStateDispatchMessage

from economic_dispatch.simulation.tests.message_generators_commitments import ManagerMessageGenerator, \
    ResourceForecastPowerMessageGenerator, PriceForecastMessageGenerator, ResourceStateMessageGenerator, \
    DispatchMessageGenerator, InputMessageGenerator, LFMMarketResultMessageGenerator, CustomerInfoMessageGenerator, \
    ReadyMessageGenerator

# test data
from economic_dispatch.simulation.tests.message_generators_commitments import DISPATCHES, GEN_FORECASTS, \
    LOAD_FORECASTS, STORAGE_STATES, PRICE_FORECASTS, NOT_OUR_GEN_FORECASTS, time_str, HORIZON, LFM_RESULTS
...
DEMO_INPUT_GENERATORS = [
    ...
    ResourceForecastPowerMessageGenerator(simulation_id=SIMULATION_ID, process_id="CHP2",
                                          forecasts=NOT_OUR_GEN_FORECASTS),
    ...
    CustomerInfoMessageGenerator(simulation_id=SIMULATION_ID, process_id="LFM1", resource_id=["Battery"],
                                 customer_id=["id1"]),
    ReadyMessageGenerator(simulation_id=SIMULATION_ID, process_id="LFM1"),
]

ED_COMPONENT_NAME = "ED-test-commitments"
    ...
    "ProcessParameters": {
        "EconomicDispatch": {
            ED_COMPONENT_NAME: {
                ...
                # Commitment time in environment variables below
            }
        },
...
os.environ[COMMITMENT_TIME] = str("12:00")
...
os.environ[SKIP_OPEN_OFFERS] = str("True")
...
class TestEconomicDispatchComponent(TestAbstractSimulationComponent):
    ...
    manager_message_generator = ManagerMessageGenerator(simulation_id, test_manager_name,
                                                        start_time=to_utc_datetime_object(time_str))
    ...
    def get_expected_messages(self, component_message_generator: MessageGenerator, epoch_number: int,
        ...
        return [
            (component_message_generator.get_dispatch_message(epoch_number, triggering_message_ids),
             "ResourceForecastState." + os.environ[RESOURCE_TYPE]),
            (component_message_generator.get_status_message(epoch_number, triggering_message_ids), "Status.Ready")
        ]
    ...
    async def epoch_tester(self, epoch_number: int, test_component: AbstractSimulationComponent,
                           message_client: RabbitmqClient,
        ...
        # Manager messages
        if epoch_number == 0:
            manager_message = self.__class__.manager_message_generator.get_simulation_state_message(True)
        else:
            manager_message = self.__class__.manager_message_generator. \
                get_epoch_message(epoch_number, [component_message_generator.latest_message_id])

        # Component inputs
        component_inputs = self.get_input_messages(input_message_generator, epoch_number, [manager_message.message_id])
        ...
        if epoch_number == 1:
            for input_message, topic_name in component_inputs:
                if isinstance(input_message, InitCISCustomerInfoMessage):
                    await send_message(message_client, input_message, topic_name)
            await asyncio.sleep(self.__class__.long_wait)

        # Resources
        for input_message, topic_name in component_inputs:
            if not isinstance(input_message, StatusMessage) and \
                    not isinstance(input_message, InitCISCustomerInfoMessage):
                await send_message(message_client, input_message, topic_name)
        ...
        # LFM Status message
        for input_message, topic_name in component_inputs:
            if isinstance(input_message, StatusMessage):
                await send_message(message_client, input_message, topic_name)
    ...
    async def test_normal_simulation(self):
        ...
        for epoch_number in range(0, self.__class__.normal_simulation_epochs + 1):
            with self.subTest(epoch_number=epoch_number):
                await self.epoch_tester(epoch_number, test_component, message_client, message_storage,
                                        component_message_generator, input_message_generator)
    ...
    def compare_dispatch_message(self, first_message: ResourceForecastStateDispatchMessage,
                                 second_message: ResourceForecastStateDispatchMessage):
        ...
        for name in first_dispatch_block.keys():
            ...
            self.assertEqual(first_ts_block.series['RealPower'].unit_of_measure,
                             second_ts_block.series['RealPower'].unit_of_measure)

    def compare_message(self, first_message: AbstractMessage, second_message: AbstractMessage) -> bool:
        """Just checking dispatch lengths"""

        if super().compare_message(first_message, second_message):
            return True

        if isinstance(first_message, ResourceForecastStateDispatchMessage):
            for dispatch in first_message.dispatch.items():
                # Returns tuple : [1] = TimeSeriesBlock
                dispatch_values = dispatch[1]
                if isinstance(dispatch_values, TimeSeriesBlock):
                    if len(dispatch_values.series['RealPower'].values) != HORIZON:
                        return False
                else:
                    return False
            return True
